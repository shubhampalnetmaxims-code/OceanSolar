...
type ActiveTab = 'home' | 'profile' | 'support' | 'inquiryForm' | 'status';
type AdminScreen = 'login' | 'dashboard';
type AdminActiveTab = 'dashboard' | 'inquiries' | 'customers' | 'support';
type NotificationType = 'success' | 'error' | 'info';

const initialInquiryState = {
    propertyType: '', propertyTypeOther: '', ownership: '', propertyAddress: '',
    roofType: '', roofTypeOther: '', roofOrientation: [] as string[], roofArea: '', roofAreaNotSure: false, roofCondition: '',
    roofInstallYear: '', roofAge: '', leaksLast5Years: '', leaksDescription: '',
    electricityProvider: '', connectionType: '', connectionTypeOther: '', avgMonthlyBill: '', avgMonthlyConsumption: '',
    meterLocation: '', meterLocationOther: '',
    systemType: '', backupRequirement: '', batteryType: '', desiredCapacity: '', budget: '',
    roofAccess: '', shadeOnRoof: '', preferredInstallDate: '', unavailableFromDate: '', unavailableToDate: '',
    additionalNotes: '', consent: false, signature: '', signatureDate: ''
};
type InquiryFormState = typeof initialInquiryState;
type InquiryKeys = keyof InquiryFormState;

type FileWithPreview = { file: File, preview: string };
type FilesData = { [key: string]: { name: string, type: string }[] };
const initialProfileState = { fullName: '', phone: '', address: '', city: '', postalCode: '', country: '', state: '' };
...
type InquiryStatus = 'Submitted' | 'In Review' | 'Approved' | 'Rejected' | 'Pending' | 'Confirmed';
...
const INQUIRY_FORM_SECTIONS = [
  { title: 'Property Information', fields: ['propertyType', 'propertyTypeOther', 'ownership', 'propertyAddress'] },
  { title: 'Roof Details', fields: ['roofType', 'roofTypeOther', 'roofOrientation', 'roofArea', 'roofAreaNotSure', 'roofCondition', 'roofInstallYear', 'leaksLast5Years', 'leaksDescription'] },
  { title: 'Electrical & Utility Information', fields: ['electricityProvider', 'connectionType', 'connectionTypeOther', 'avgMonthlyBill', 'avgMonthlyConsumption', 'meterLocation', 'meterLocationOther'] },
  { title: 'System Preferences', fields: ['systemType', 'backupRequirement', 'batteryType', 'desiredCapacity', 'budget'] },
  { title: 'Site Access & Installation Details', fields: ['roofAccess', 'shadeOnRoof', 'preferredInstallDate', 'unavailableFromDate', 'unavailableToDate'] },
...
const setupMockData = () => {
    ...
    const mockProfiles = {
        'test@gmail.com': { fullName: 'Alex Ray', phone: '555-123-4567', address: '123 Solar Way', city: 'Sunnyvale', postalCode: '94086', country: 'USA', state: 'CA' },
        'jane.doe@example.com': { fullName: 'Jane Doe', phone: '555-0101-2222', address: '456 Solar Ave', city: 'Sunville', postalCode: '12345', country: 'USA', state: 'FL' },
        'john.smith@example.com': { fullName: 'John Smith', phone: '555-0202-3333', address: '789 Power St', city: 'Gridley', postalCode: '67890', country: 'USA', state: 'TX' }
    };
    const mockInquiries: InquirySubmission[] = [
        {
            id: 'INQ-1678886400000', submittedAt: new Date('2024-07-21T09:00:00Z').toISOString(), userEmail: 'test@gmail.com', status: 'Submitted', messages: [],
            formData: { ...initialInquiryState, propertyType: 'Residential', consent: true, signature: 'Alex Ray', signatureDate: '2024-07-21' },
            filesData: {}
        },
        {
            id: 'INQ-1678886400001', submittedAt: new Date('2024-07-15T10:30:00Z').toISOString(), userEmail: 'jane.doe@example.com', status: 'In Review', messages: [{ sender: 'admin', text: 'Thank you for your submission. We are reviewing your details.', timestamp: new Date('2024-07-15T11:00:00Z').toISOString() }],
            formData: { ...initialInquiryState, propertyType: 'Residential', ownership: 'Owned', roofType: 'RCC (Concrete)', roofOrientation: ['South', 'West'], roofArea: '1200', roofCondition: 'Good', electricityProvider: 'City Power Corp', connectionType: 'Three Phase', avgMonthlyBill: '150', systemType: 'Hybrid', budget: '15000', consent: true, signature: 'Jane Doe', signatureDate: '2024-07-15' },
        ...
        {
            id: 'INQ-1678886400002', submittedAt: new Date('2024-07-18T14:00:00Z').toISOString(), userEmail: 'john.smith@example.com', status: 'Confirmed', messages: [],
            formData: { ...initialInquiryState, propertyType: 'Commercial', ownership: 'Leased', roofType: 'Metal Sheet', roofOrientation: ['Flat'], roofArea: '5000', roofCondition: 'Good', electricityProvider: 'State Utility', connectionType: 'Three Phase', avgMonthlyBill: '800', avgMonthlyConsumption: '4500', systemType: 'On-Grid', budget: '50000', consent: true, signature: 'John Smith', signatureDate: '2024-07-18' },
    ...
    const mockNotifications: NotificationItem[] = [
      { id: 'NOT-1', recipient: 'admin', title: 'New Inquiry', message: 'New inquiry received from test@gmail.com', type: 'info', read: false, timestamp: new Date().toISOString() },
      { id: 'NOT-2', recipient: 'test@gmail.com', title: 'Welcome', message: 'Welcome to Ocean Solar! Start your journey by submitting an inquiry.', type: 'success', read: false, timestamp: new Date().toISOString() }
    ];
    try {
        localStorage.setItem('oceanSolarProfiles', JSON.stringify(mockProfiles));
        localStorage.setItem('oceanSolarAllInquiries', JSON.stringify(mockInquiries));
...
type TextInputProps = { label: string; name: string; value: string; onChange: (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) => void; placeholder?: string; type?: string; required?: boolean; disabled?: boolean; };
...
type StatusPillProps = { status: InquiryStatus };
type InquiryDetailViewCustomerProps = { inquiry: InquirySubmission; onBack: () => void; onSendMessage: (id: string, text: string) => void; };
type SupportScreenProps = { supportTickets: SupportTicket[]; inquiries: InquirySubmission[]; onCreateTicket: (subject: string, message: string) => void; onInquiryClick: (inquiry: InquirySubmission) => void; };
type AdminMessageFormProps = { itemId: string; itemType: 'inquiry' | 'ticket'; onSendMessage: (itemId: string, messageText: string, itemType: 'inquiry' | 'ticket') => void; };
type AdminInquiryDetailViewProps = { inquiry: InquirySubmission; customerProfile: Profile | undefined; onBack: () => void; onStatusChange: (inquiryId: string, newStatus: InquiryStatus) => void; };

// --- Stable Form Field & UI Components ---
const TextInput: React.FC<TextInputProps> = ({ label, name, value, onChange, placeholder, type = 'text', required = false, disabled = false }) => ( <div className="space-y-2"> <label htmlFor={name} className="text-sm text-gray-300">{label}{required && <span className="text-red-400">*</span>}</label> <input type={type} name={name} id={name} placeholder={placeholder || label} value={value} onChange={onChange} required={required} disabled={disabled} className="w-full bg-gray-800/50 border border-gray-700 rounded-lg py-3 px-4 text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500 disabled:bg-gray-900/70 disabled:cursor-not-allowed" /> </div> );
...
const AdminInquiryDetailView: React.FC<AdminInquiryDetailViewProps> = ({ inquiry, customerProfile, onBack, onStatusChange }) => { 
    const fullAddress = customerProfile ? [customerProfile.address, customerProfile.city, customerProfile.state, customerProfile.postalCode, customerProfile.country].filter(Boolean).join(', ') : 'N/A';
    return ( 
    <div className="flex-1 overflow-y-auto p-6 bg-gray-900 text-white"> 
        <div className="flex items-center mb-6"> 
            <button onClick={onBack} className="flex items-center gap-2 text-gray-300 hover:text-white transition-colors p-2 -ml-2"> 
                <ChevronLeftIcon className="w-6 h-6" /> <span>Back to Inquiries</span> 
            </button> 
        </div> 
            ...
            <div className="col-span-3 lg:col-span-2 space-y-6"> 
                <div className="bg-gray-800 border border-gray-700 rounded-lg p-6"> 
                    <div className="flex justify-between items-start"> 
                        <div>
                            <h2 className="text-2xl font-bold text-white">{inquiry.id}</h2>
                            <p className="text-sm text-gray-400">Submitted on {new Date(inquiry.submittedAt).toLocaleString()}</p>
                        </div> 
                        <StatusPill status={inquiry.status} /> 
                    </div> 
                    <div className="mt-4 pt-4 border-t border-gray-700/50 text-sm">
                        <h3 className="font-semibold text-lg text-blue-400 mb-2">Customer Details</h3>
                        <p><strong className="text-gray-400 w-24 inline-block">Name:</strong> {customerProfile?.fullName || 'N/A'}</p>
                        <p><strong className="text-gray-400 w-24 inline-block">Email:</strong> {inquiry.userEmail}</p>
                        <p><strong className="text-gray-400 w-24 inline-block">Phone:</strong> {customerProfile?.phone || 'N/A'}</p>
                        <p><strong className="text-gray-400 w-24 inline-block">Address:</strong> {fullAddress}</p>
                    </div> 
                </div>
<div className="bg-gray-800 border border-gray-700 rounded-lg">
    <h3 className="text-lg font-semibold text-blue-400 p-4 border-b border-gray-700">Submitted Information</h3>
    <div className="p-6 space-y-6 text-sm">
        {INQUIRY_FORM_SECTIONS.map(section => {
            const sectionFields = section.fields
                .map(fieldKey => {
                    const value = inquiry.formData[fieldKey as keyof InquiryFormState];
 ...
 <div className="bg-gray-800 border border-gray-700 rounded-lg"><h3 className="text-lg font-semibold text-blue-400 p-4 border-b border-gray-700">Uploaded Files</h3><div className="p-6">{Object.keys(inquiry.filesData).length > 0 ? Object.entries(inquiry.filesData).map(([key, files]) => { if(!files || files.length === 0) return null; const label = key.replace(/([A-Z])/g, ' $1').replace(/^./, str => str.toUpperCase()); return (<div key={key} className="mb-4"><strong className="text-gray-300 capitalize text-base">{label}</strong><div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 mt-2">{files.map((file, idx) => ( <div key={idx} className="bg-gray-900/70 p-2 rounded-md flex flex-col items-center justify-center text-center aspect-square border border-gray-600" title={file.name}> <CameraIcon className="w-8 h-8 text-gray-400 mb-2 shrink-0" /> <p className="text-white text-xs break-all leading-tight">{file.name}</p> </div> ))}</div></div>)}) : <p className="text-gray-400">No files were uploaded.</p>}</div></div> </div> 
            <div className="col-span-3 lg:col-span-1 space-y-6"> 
                <div className="bg-gray-800 border border-gray-700 rounded-lg p-6">
                    <h3 className="text-lg font-semibold text-blue-400 mb-4">Update Status</h3>
                    <select value={inquiry.status} onChange={(e) => onStatusChange(inquiry.id, e.target.value as InquiryStatus)} className="w-full bg-gray-900 border border-gray-600 rounded-lg p-2 text-white">
...
const App: React.FC = () => {
  ...
  const [allInquiries, setAllInquiries] = useState<InquirySubmission[]>([]);
  ...
  const [adminSelectedInquiry, setAdminSelectedInquiry] = useState<InquirySubmission | null>(null);
  ...
  const loadDataForCurrentUser = (userEmail: string) => {
    try {
      const allInquiriesRaw = localStorage.getItem('oceanSolarAllInquiries');
      if (allInquiriesRaw) {
        const allInquiriesParsed: InquirySubmission[] = JSON.parse(allInquiriesRaw);
        setInquiries(allInquiriesParsed.filter(e => e.userEmail === userEmail));
      }
  ...
  const handleInquiryChange = (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement>) => {
    ...
    if (type === 'checkbox') {
        ...
        if (name === 'roofAreaNotSure') {
            setInquiryForm(prev => ({ ...prev, roofAreaNotSure: checked, roofArea: checked ? '' : prev.roofArea }));
        } else {
            setInquiryForm(prev => ({ ...prev, [name]: checked }));
        }
    } else {
        if (name === 'roofArea') {
            setInquiryForm(prev => ({ ...prev, [name]: value, roofAreaNotSure: false }));
        } else {
            setInquiryForm(prev => ({ ...prev, [name]: value }));
        }
    }
  };
  const handleMultiSelectChange = (e: React.ChangeEvent<HTMLSelectElement>) => { const { name, options } = e.target; const value: string[] = []; for (let i = 0, l = options.length; i < l; i++) { if (options[i].selected) { value.push(options[i].value); } } setInquiryForm(prev => ({ ...prev, [name]: value })); };
  ...
  const handleInquirySubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const newInquiry: InquirySubmission = { id: `INQ-${Date.now()}`, submittedAt: new Date().toISOString(), formData: inquiryForm, filesData: filesData, userEmail: email, status: 'Submitted', messages: [] };
    try {
        const allInquiriesRaw = localStorage.getItem('oceanSolarAllInquiries');
        const allInquiries = allInquiriesRaw ? JSON.parse(allInquiriesRaw) : [];
        allInquiries.unshift(newInquiry);
        localStorage.setItem('oceanSolarAllInquiries', JSON.stringify(allInquiries));
        setInquiries(prev => [newInquiry, ...prev]);
        createNotification('admin', 'New Inquiry Received', `New inquiry ${newInquiry.id} from ${email}`, 'info');
    } catch (error) { 
  ...
  const handleLocateProperty = async () => {
    const { location, address } = await locateUser((addressUpdate) => {
      const fullAddress = [addressUpdate.address, addressUpdate.city, addressUpdate.state, addressUpdate.postalCode, addressUpdate.country]
        .filter(Boolean).join(', ');
      setInquiryForm(prev => ({ ...prev, propertyAddress: fullAddress }));
    });
    if (location) {
  ...
  const handleAdminLogin = () => {
    try {
      ...
      setAllInquiries(JSON.parse(localStorage.getItem('oceanSolarAllInquiries') || '[]'));
  ...
  const handleAdminLogout = () => { setAppState('auth'); setAuthScreen('welcome'); setAdminScreen('login'); setAllInquiries([]); setAllCustomers({}); setAdminSelectedInquiry(null); };
  const handleStatusChange = (inquiryId: string, newStatus: InquiryStatus) => {
    const updatedInquiries = allInquiries.map(inq => inq.id === inquiryId ? { ...inq, status: newStatus } : inq);
    setAllInquiries(updatedInquiries);
    if(adminSelectedInquiry?.id === inquiryId) { setAdminSelectedInquiry(prev => prev ? {...prev, status: newStatus} : null); }
    localStorage.setItem('oceanSolarAllInquiries', JSON.stringify(updatedInquiries));
    showNotification(`Inquiry ${inquiryId} status updated to ${newStatus}.`, 'success');
    
    const inquiry = allInquiries.find(e => e.id === inquiryId);
    if(inquiry) createNotification(inquiry.userEmail, 'Inquiry Update', `Your inquiry ${inquiry.id} status is now ${newStatus}.`, 'info');
  };
  const handleAdminSendMessage = (itemId: string, messageText: string, itemType: 'inquiry' | 'ticket') => {
    if (!messageText.trim()) return;
    const newMessage: Message = { sender: 'admin', text: messageText, timestamp: new Date().toISOString() };
    if (itemType === 'inquiry') {
      const updatedInquiries = allInquiries.map(inq => inq.id === itemId ? { ...inq, messages: [...inq.messages, newMessage] } : inq);
      setAllInquiries(updatedInquiries);
      if(adminSelectedInquiry?.id === itemId) { setAdminSelectedInquiry(prev => prev ? {...prev, messages: [...prev.messages, newMessage]} : null); }
      localStorage.setItem('oceanSolarAllInquiries', JSON.stringify(updatedInquiries));
      
      const inquiry = allInquiries.find(e => e.id === itemId);
      if(inquiry) createNotification(inquiry.userEmail, 'New Message', `Admin replied to inquiry ${itemId}.`, 'info');
  ...
  const handleCustomerReply = (inquiryId: string, messageText: string) => {
    if (!messageText.trim()) return;
    const newMessage: Message = { sender: 'customer', text: messageText, timestamp: new Date().toISOString() };
    const allInquiriesRaw = localStorage.getItem('oceanSolarAllInquiries') || '[]';
    const allInquiriesParsed: InquirySubmission[] = JSON.parse(allInquiriesRaw);
    const updatedInquiries = allInquiriesParsed.map(inq => inq.id === inquiryId ? { ...inq, messages: [...inq.messages, newMessage] } : inq);
    localStorage.setItem('oceanSolarAllInquiries', JSON.stringify(updatedInquiries));
    setInquiries(updatedInquiries.filter(e => e.userEmail === email));
    setSelectedInquiry(updatedInquiries.find(e => e.id === inquiryId) || null);
    createNotification('admin', 'Inquiry Message', `User replied to inquiry ${inquiryId}`, 'info');
  };
  ...
  const handleSupportInquiryClick = (inquiry: InquirySubmission) => {
    setActiveTab('status');
  ...
  const renderHomeScreen = () => ( 
    <div className="p-8 text-white animate-fade-in relative"> 
        ...
        <div className="space-y-4"> 
            <div onClick={() => setActiveTab('inquiryForm')} className="bg-gray-800/70 p-6 rounded-lg border border-gray-700 cursor-pointer hover:bg-gray-700/70 transition-colors"><h2 className="text-xl font-semibold">Solar Inquiry</h2><p className="text-gray-400 mt-1">Start a new inquiry for solar panel installation.</p></div> 
            <div onClick={() => setActiveTab('status')} className="bg-gray-800/70 p-6 rounded-lg border border-gray-700 cursor-pointer hover:bg-gray-700/70 transition-colors"><h2 className="text-xl font-semibold">Check Status</h2><p className="text-gray-400 mt-1">View your submitted inquiries and messages.</p></div> 
  ...
  const renderInquiryForm = () => {
    ...
    return (
        <div className="p-6 text-white animate-fade-in h-full flex flex-col">
            <div className="flex items-center mb-4"><button onClick={() => setActiveTab('home')} className="p-2 -ml-2 text-gray-300 hover:text-white"><ChevronLeftIcon className="w-6 h-6" /></button><h1 className="text-xl font-bold ml-2">Solar Inquiry Form</h1></div>
            <div className="flex-grow overflow-y-auto pr-2 -mr-4 space-y-6 pb-4">
                <form id="inquiryForm" onSubmit={handleInquirySubmit}> <div className="space-y-6">
                    <div className="space-y-4"><h2 className="text-lg font-semibold text-blue-400 border-b border-gray-700 pb-2">Property Information</h2><SelectInput label="Property Type" name="propertyType" value={inquiryForm.propertyType} onChange={handleInquiryChange} required><option value="">Select...</option><option>Residential</option><option>Commercial</option><option>Industrial</option><option value="Other">Other</option></SelectInput>{inquiryForm.propertyType === 'Other' && <TextInput label="Please specify" name="propertyTypeOther" value={inquiryForm.propertyTypeOther} onChange={handleInquiryChange} required />}<SelectInput label="Ownership" name="ownership" value={inquiryForm.ownership} onChange={handleInquiryChange} required><option value="">Select...</option><option>Owned</option><option>Rented</option><option>Leased</option></SelectInput>
                    <div className="relative">
                        <TextInput label="Property Address (if different)" name="propertyAddress" value={inquiryForm.propertyAddress} onChange={handleInquiryChange} placeholder="Enter full property address" />
                    ...
                    <div className="space-y-4"><h2 className="text-lg font-semibold text-blue-400 border-b border-gray-700 pb-2">Roof Details</h2><SelectInput label="Roof Type" name="roofType" value={inquiryForm.roofType} onChange={handleInquiryChange} required><option value="">Select...</option><option>RCC (Concrete)</option><option>Metal Sheet</option><option>Asbestos</option><option>Tile</option><option value="Other">Other</option></SelectInput>{inquiryForm.roofType === 'Other' && <TextInput label="Please specify" name="roofTypeOther" value={inquiryForm.roofTypeOther} onChange={handleInquiryChange} required />}<div className="space-y-2"><label htmlFor="roofOrientation" className="text-sm text-gray-300">Roof Orientation <span className="text-red-400">*</span></label><select multiple name="roofOrientation" id="roofOrientation" value={inquiryForm.roofOrientation} onChange={handleMultiSelectChange} required className="w-full h-32 bg-gray-800/50 border border-gray-700 rounded-lg py-3 px-4 text-white focus:outline-none focus:ring-2 focus:ring-blue-500 custom-multiselect"><option>East</option><option>West</option><option>South</option><option>North</option><option>Flat</option></select></div>
                    <div className="flex items-end gap-4">
                        <div className="flex-grow">
                            <TextInput label="Roof Area Available (sq. ft.)" name="roofArea" value={inquiryForm.roofArea} onChange={handleInquiryChange} disabled={inquiryForm.roofAreaNotSure} />
                        </div>
                        <div className="flex items-center pb-3">
                            <input type="checkbox" id="roofAreaNotSure" name="roofAreaNotSure" checked={inquiryForm.roofAreaNotSure} onChange={handleInquiryChange} className="h-4 w-4 rounded border-gray-600 bg-gray-800 text-blue-500 focus:ring-blue-500" />
                            <label htmlFor="roofAreaNotSure" className="ml-2 text-sm text-gray-300">Not Sure</label>
                        </div>
                    </div>
                    <SelectInput label="Roof Condition" name="roofCondition" value={inquiryForm.roofCondition} onChange={handleInquiryChange}><option value="">Select...</option><option>Good</option><option>Average</option><option>Poor</option></SelectInput><SelectInput label="Year Roof Was Installed" name="roofInstallYear" value={inquiryForm.roofInstallYear} onChange={handleInquiryChange}><option value="">Select Year</option>{years.map(year => (<option key={year} value={year}>{year}</option>))}</SelectInput><div className="space-y-2"><label className="text-sm text-gray-300">Leaks in Last 5 Years?</label><div className="flex gap-4 pt-1"><label className="flex items-center gap-2"><input type="radio" name="leaksLast5Years" value="Yes" onChange={handleInquiryChange} checked={inquiryForm.leaksLast5Years === "Yes"} className="h-4 w-4 bg-gray-800 border-gray-600 text-blue-500" /> Yes</label><label className="flex items-center gap-2"><input type="radio" name="leaksLast5Years" value="No" onChange={handleInquiryChange} checked={inquiryForm.leaksLast5Years === "No"} className="h-4 w-4 bg-gray-800 border-gray-600 text-blue-500" /> No</label></div></div>{inquiryForm.leaksLast5Years === 'Yes' && <div className="space-y-2"><label htmlFor="leaksDescription" className="text-sm text-gray-300">If yes, describe:</label><textarea name="leaksDescription" id="leaksDescription" value={inquiryForm.leaksDescription} onChange={handleInquiryChange} rows={3} className="w-full bg-gray-800/50 border border-gray-700 rounded-lg py-2 px-4 text-white"></textarea></div>}<FileUpload label="Upload Roof Photos" name="roofPhotos" files={formFiles.roofPhotos} onChange={handleFileChange} onRemove={handleRemoveFile} multiple /><FileUpload label="Upload Exterior Roof Overview" name="exteriorRoofPhoto" files={formFiles.exteriorRoofPhoto} onChange={handleFileChange} onRemove={handleRemoveFile} multiple /></div>
                    <div className="space-y-4"><h2 className="text-lg font-semibold text-blue-400 border-b border-gray-700 pb-2">Electrical Information</h2><TextInput label="Electricity Provider" name="electricityProvider" value={inquiryForm.electricityProvider} onChange={handleInquiryChange} required />
                    <SelectInput label="Connection Type" name="connectionType" value={inquiryForm.connectionType} onChange={handleInquiryChange}><option value="">Select...</option><option>Single Phase</option><option>Three Phase</option><option>Not Sure</option><option value="Other">Other</option></SelectInput>
                    {inquiryForm.connectionType === 'Other' && <TextInput label="Please specify connection type" name="connectionTypeOther" value={inquiryForm.connectionTypeOther} onChange={handleInquiryChange} required />}
                    <TextInput label="Avg Monthly Bill (₹/$)" name="avgMonthlyBill" value={inquiryForm.avgMonthlyBill} onChange={handleInquiryChange} type="number" required /><TextInput label="Avg Monthly Consumption (kWh)" name="avgMonthlyConsumption" value={inquiryForm.avgMonthlyConsumption} onChange={handleInquiryChange} type="number" /><SelectInput label="Meter Location" name="meterLocation" value={inquiryForm.meterLocation} onChange={handleInquiryChange}><option value="">Select...</option><option>Outside</option><option>Inside</option><option value="Other">Other</option></SelectInput>{inquiryForm.meterLocation === 'Other' && <TextInput label="Please specify" name="meterLocationOther" value={inquiryForm.meterLocationOther} onChange={handleInquiryChange} required />}<FileUpload label="Upload Electricity Bill" name="electricityBill" files={formFiles.electricityBill} onChange={handleFileChange} onRemove={handleRemoveFile} required multiple /><FileUpload label="Upload Utility Meter Photo" name="utilityMeterPhoto" files={formFiles.utilityMeterPhoto} onChange={handleFileChange} onRemove={handleRemoveFile} multiple required /><FileUpload label="Upload Main Service Panel" name="mainPanelPhoto" files={formFiles.mainPanelPhoto} onChange={handleFileChange} onRemove={handleRemoveFile} multiple required /><FileUpload label="Upload Interior Attic Photo" name="atticPhoto" files={formFiles.atticPhoto} onChange={handleFileChange} onRemove={handleRemoveFile} multiple required /></div>
                    <div className="space-y-4"><h2 className="text-lg font-semibold text-blue-400 border-b border-gray-700 pb-2">System Preferences</h2><SelectInput label="System Type" name="systemType" value={inquiryForm.systemType} onChange={handleInquiryChange}><option value="">Select...</option><option>On-Grid</option><option>Off-Grid</option><option>Hybrid</option><option>Not Sure</option></SelectInput><SelectInput label="Backup Requirement" name="backupRequirement" value={inquiryForm.backupRequirement} onChange={handleInquiryChange}><option value="">Select...</option><option>Full</option><option>Partial</option><option>None</option></SelectInput><SelectInput label="Battery Type" name="batteryType" value={inquiryForm.batteryType} onChange={handleInquiryChange}><option value="">Select...</option><option>Lithium-ion</option><option>Lead-acid</option><option>Not Sure</option></SelectInput><TextInput label="Desired Capacity (if known)" name="desiredCapacity" value={inquiryForm.desiredCapacity} onChange={handleInquiryChange} placeholder="e.g., 5 kW" /><TextInput label="Budget Range" name="budget" value={inquiryForm.budget} onChange={handleInquiryChange} placeholder="e.g., ₹3,00,000" /></div>
                    <div className="space-y-4"><h2 className="text-lg font-semibold text-blue-400 border-b border-gray-700 pb-2">Site & Installation</h2><SelectInput label="Roof Access" name="roofAccess" value={inquiryForm.roofAccess} onChange={handleInquiryChange}><option value="">Select...</option><option>Easy</option><option>Limited</option><option>Ladder Required</option></SelectInput><SelectInput label="Shade on Roof" name="shadeOnRoof" value={inquiryForm.shadeOnRoof} onChange={handleInquiryChange}><option value="">Select...</option><option>None</option><option>Partial</option><option>Full</option></SelectInput><TextInput label="Preferred Install Date" name="preferredInstallDate" value={inquiryForm.preferredInstallDate} onChange={handleInquiryChange} type="date" />
                    <div className="space-y-2"><label className="text-sm text-gray-300">Unavailable Dates (if any)</label><div className="grid grid-cols-2 gap-4"><TextInput label="From" name="unavailableFromDate" value={inquiryForm.unavailableFromDate} onChange={handleInquiryChange} type="date" /><TextInput label="To" name="unavailableToDate" value={inquiryForm.unavailableToDate} onChange={handleInquiryChange} type="date" /></div></div>
                    <FileUpload label="Upload Site / Surroundings Photos" name="sitePhotos" files={formFiles.sitePhotos} onChange={handleFileChange} onRemove={handleRemoveFile} multiple /></div>
                    <div className="space-y-4"><h2 className="text-lg font-semibold text-blue-400 border-b border-gray-700 pb-2">Additional Notes</h2><textarea name="additionalNotes" value={inquiryForm.additionalNotes} onChange={handleInquiryChange} rows={4} className="w-full bg-gray-800/50 border border-gray-700 rounded-lg py-2 px-4 text-white" placeholder="Any other relevant information?"></textarea></div>
                    <div className="space-y-4"><h2 className="text-lg font-semibold text-blue-400 border-b border-gray-700 pb-2">Signature & Consent</h2><div className="flex items-start gap-3"><input type="checkbox" name="consent" id="consent" checked={inquiryForm.consent} onChange={handleInquiryChange} required className="mt-1 h-4 w-4 rounded border-gray-600 bg-gray-800 text-blue-500" /><label htmlFor="consent" className="text-sm text-gray-300">I confirm the information is true and authorize a site inspection.</label></div><TextInput label="Signature (Type full name)" name="signature" value={inquiryForm.signature} onChange={handleInquiryChange} required /><TextInput label="Date" name="signatureDate" value={inquiryForm.signatureDate} onChange={handleInquiryChange} type="date" required /></div>
                </div></form>
            </div>
            <button type="submit" form="inquiryForm" className="w-full bg-blue-500 hover:bg-blue-600 text-white font-bold py-3 px-4 rounded-lg text-lg transition-transform transform active:scale-95 mt-4 flex-shrink-0">Submit Inquiry</button>
        </div>
    );
  };
  ...
  const renderDashboard = () => {
    ...
    switch (activeTab) {
      ...
      case 'inquiryForm': content = renderInquiryForm(); break;
      case 'status': content = renderStatusScreen(); break;
      case 'support': content = <SupportScreen supportTickets={supportTickets} inquiries={inquiries} onCreateTicket={handleCreateSupportTicket} onInquiryClick={handleSupportInquiryClick} />; break;
  ...
  const renderAdminDashboard = () => {
    const renderContent = () => { switch (adminActiveTab) { case 'dashboard': return renderAdminOverview(); case 'inquiries': return renderAdminInquiries(); case 'customers': return renderAdminCustomers(); case 'support': return renderAdminSupport(); default: return null; } };
        ...
        <div className="w-64 bg-gray-900/80 border-r border-gray-700 flex flex-col justify-between">
          <div> <div className="p-4 border-b border-gray-700 flex items-center gap-3"><LogoIcon className="w-10 h-10" /><span className="font-semibold text-lg">Admin Panel</span></div> 
            <nav className="mt-4 p-2 space-y-1"> 
              ...
              <button onClick={() => handleTabClick('inquiries')} className={`w-full text-left text-sm px-4 py-3 rounded-md flex items-center gap-3 transition-colors ${adminActiveTab === 'inquiries' ? 'bg-blue-500/30 text-blue-300' : 'text-gray-400 hover:bg-gray-700/50'}`}><MailIcon className="w-5 h-5" /><span>Inquiries</span></button> 
  ...
  const renderAdminOverview = () => {
      ...
      const pendingInquiries = allInquiries.filter(e => e.status === 'Submitted' || e.status === 'Pending').length;
      const totalCustomers = Object.keys(allCustomers).length;
      const openTickets = allSupportTickets.filter(t => t.status === 'Open').length;
      
      const statusCounts = allInquiries.reduce((acc, curr) => {
        ...
        <div className="flex-1 overflow-y-auto p-8 space-y-8 bg-gray-900">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                <div className="bg-gray-800 border border-gray-700 p-6 rounded-xl shadow-sm">
                    <div className="flex items-center justify-between mb-4"><h3 className="text-gray-400 font-medium">Total Inquiries</h3><div className="p-2 bg-blue-500/10 rounded-lg"><MailIcon className="w-6 h-6 text-blue-500" /></div></div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                <div className="bg-gray-800 border border-gray-700 rounded-xl p-6">
                    <h3 className="text-lg font-bold text-white mb-6">Inquiry Status Breakdown</h3>
                    <div className="space-y-4">
                        {statuses.map(status => {
                ...
                <div className="bg-gray-800 border border-gray-700 rounded-xl p-6">
                     <div className="flex justify-between items-center mb-6"><h3 className="text-lg font-bold text-white">Recent Inquiries</h3><button onClick={() => setAdminActiveTab('inquiries')} className="text-sm text-blue-400 hover:text-blue-300">View All</button></div>
                     <div className="space-y-4">
                        {allInquiries.slice(0, 5).map(inq => (
                            <div key={inq.id} className="flex items-center justify-between p-3 bg-gray-900/50 rounded-lg border border-gray-700/50">
                                <div className="flex items-center gap-3">
                                    ...
                                    <div>
                                        <p className="text-sm font-semibold text-white">{inq.userEmail}</p>
                                        <p className="text-xs text-gray-500">{inq.id}</p>
                                    </div>
                                </div>
                                <StatusPill status={inq.status} />
                            </div>
                        ))}
                        {allInquiries.length === 0 && <p className="text-gray-500 text-center py-4">No inquiries yet.</p>}
                     </div>
                </div>
            </div>
        </div>
  ...
  const renderAdminInquiries = () => {
    if (adminSelectedInquiry) {
        const customerProfile = allCustomers[adminSelectedInquiry.userEmail];
        return <AdminInquiryDetailView
            inquiry={adminSelectedInquiry}
            customerProfile={customerProfile}
            onBack={() => setAdminSelectedInquiry(null)}
    ...
    const filteredInquiries = allInquiries.filter(inq => {
        ...
        return (
            inq.id.toLowerCase().includes(searchTerm) ||
    ...
    return (
        <div className="flex-1 flex flex-col p-6 space-y-4 bg-gray-900 overflow-hidden">
            <h1 className="text-3xl font-bold text-gray-100 shrink-0">Inquiry Forms</h1>
            <input
                type="text"
                placeholder="Search by Inquiry ID or email..."
            ...
            {filteredInquiries.length === 0 ? (
                <div className="flex-1 flex items-center justify-center text-gray-400"><p>{adminSearchQuery ? 'No matching inquiries found.' : 'No inquiries have been submitted yet.'}</p></div>
            ) : (
                <div className="flex-1 overflow-y-auto bg-gray-800 border border-gray-700 rounded-lg">
                    <table className="w-full text-sm text-left text-gray-300">
                        <thead className="text-xs text-gray-400 uppercase bg-gray-700/50 sticky top-0">
                          <tr className="border-b border-gray-700">
                            <th scope="col" className="px-6 py-3">Inquiry ID</th>
                        ...
                        <tbody>
                            {filteredInquiries.map(inq => (
                                <tr key={inq.id} className="border-b border-gray-700 last:border-b-0 hover:bg-gray-700/50">
                                    <td className="px-6 py-4 font-medium text-blue-400 whitespace-nowrap">{inq.id}</td>
                                    <td className="px-6 py-4">{inq.userEmail}</td>
                                    <td className="px-6 py-4">{new Date(inq.submittedAt).toLocaleDateString()}</td>
                                    <td className="px-6 py-4"><StatusPill status={inq.status} /></td>
                                    <td className="px-6 py-4 text-right"><button onClick={() => setAdminSelectedInquiry(inq)} className="font-medium text-white bg-blue-600 hover:bg-blue-700 px-3 py-1.5 rounded-md text-xs">View</button></td>
                                </tr>
                            ))}
                        </tbody>
                    </table>
                        ...
                        {filteredCustomers.map(([email, profile]) => (
                            <div key={email} className="bg-gray-800 border border-gray-700 rounded-lg p-4 flex flex-col gap-2 h-fit">
                                ...
                                <div className="text-sm text-gray-300 border-t border-gray-700/50 pt-2 flex flex-col gap-1">
                                    ...
                                    <p><strong className="text-gray-400">Address:</strong> {`${profile.address || ''}${profile.city ? `, ${profile.city}` : ''}${profile.state ? `, ${profile.state}`: ''}` || 'N/A'}</p>
                                </div>
                            </div>
                        ))}
                    </div>
